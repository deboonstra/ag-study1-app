...
Currently, the web application does not allow the user to download the predicted probabilities calculated. Therefore, obtain the probabilities for external use, installation of the [`agstudy1app`](https://github.com/deboonstra/agstudy1app) is required. Then, simply call the `agstudy1app::pred_prob()` function with the injury characteristics of interest.